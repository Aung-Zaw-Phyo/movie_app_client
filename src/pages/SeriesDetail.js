        ...
        <>
            ...
            <div className="container py-10">
                ...
                <div className="">
                    ...
                    >
                        {
                            movie.series_cast.map((cast) => (
                                <SwiperSlide key={cast.person_name}>
                                    <div >
                                        <img src={cast.character_image} className="w-full rounded-lg mb-2" alt="" />
                                        <div>
                                            <div>{cast.person_name}</div>
                                            <div className="text-[#ffffffb8]">{cast.character}</div>
                                        </div>
                                    </div>
                                </SwiperSlide>
                            ))
...
const movieLoader = async (request, params) => {
    const id = params.id
    const response = await fetch(process.env.REACT_APP_API_URL + '/series/detail?id=' + id, {
        method: 'GET',
        headers: {
